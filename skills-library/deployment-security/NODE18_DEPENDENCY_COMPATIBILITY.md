...
5. **Branch divergence** — The package was removed from `your-dev-branch` but `feature-branch` still had it because the deletion was never committed as a discrete change to that branch
...
`open-graph-scraper@6.11.0` was in `server/package.json` on the `feature-branch` production branch. It was previously removed from `your-dev-branch` (development branch), but the removal never propagated to the production branch. During deployment, `npm install` pulled in `undici@7.20.0` as a transitive dependency, which crashed on Node 18.
