...
- Today's real-world example: the study app v3.0 — LightRAG crash, FFmpeg lavfi, login rate limiter. The rate limiter bug was only caught because a screenshot (the test's failure artifact) showed "Too many login attempts" — without that test, we'd have debugged auth logic for hours.
