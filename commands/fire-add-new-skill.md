...
```bash
# Read patterns from config/private-data-patterns.local.yml (UNTRACKED — you create it).
# No real patterns ship with this repo: a denylist that ships its own entries
# publishes the dossier it exists to hide. If the file is absent, this step is skipped.
PATTERNS_FILE="${PLUGIN_ROOT}/config/private-data-patterns.local.yml"
if [ ! -f "$PATTERNS_FILE" ]; then
  echo "[4.7] no local private-data patterns configured - skipping project scan"
  echo "      create config/private-data-patterns.local.yml to enable it"
fi
```
...
**Config maintenance:** Create `config/private-data-patterns.local.yml` (gitignored) and add your project-specific patterns there. Nothing real ships in this repo — the file is yours alone, and it must never be committed. Each entry takes `pattern`, `replacement`, `severity` (BLOCK or WARN), and `description`.
</step>
